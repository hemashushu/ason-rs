...
impl Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Error::MessageWithPosition(detail, position) => {
                writeln!(
                    f,
            ...
            Error::MessageWithRange(detail, range) => {
                writeln!(
                    f,
                    "Error at line: {}, column: {}",
                    range.line + 1,
    ...
    pub fn with_source(&self, source: &str) -> String {
        ...
        match self {
            ...
            Error::MessageWithPosition(detail, position) => {
                ...
                let snippet_range = calculate_snippet_range(position.index, 0, source_total_length);
                let (snippet, indented_detail) =
                    generate_snippet_and_indented_detail(&mut chars, &snippet_range, detail);
                format!("{}\n{}\n{}", msg, snippet, indented_detail)
            }
            Error::MessageWithRange(detail, range) => {
                ...
                let snippet_range =
                    calculate_snippet_range(range.index, range.length, source_total_length);
                let (snippet, indented_detail) =
                    generate_snippet_and_indented_detail(&mut chars, &snippet_range, detail);
                format!("{}\n{}\n{}", msg, snippet, indented_detail)
            }
